...
import { useAddUserToDomainMutation } from "@/mutations/useAddUserToDomainMutation";
...
function AddUserToDomain({ userId }: { userId: number }) {
  ...
  const onSuccess = () => {
    toast.success("User has been added to the domain");
    setOpen(false);
    reset();
  };
  ...
  const { mutate, isPending: addUserPending } = useAddUserToDomainMutation({
    onSuccess,
    onError,
  });
...
export default AddUserToDomain;
